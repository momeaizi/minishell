...
/*   Updated: 2022/06/23 15:33:05 by momeaizi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	replace_var_by_val(char *str, t_expand_var *exp_var)
{
	...
	while (str[++i])
	{
		...
		(p_ft_isalnum(str[i + 1]) || str[i + 1] == '_' || str[i + 1] == '?'))
		{
			p_ft_strlcpy(exp_var->new_str + j, exp_var->env_var[index].val, \
